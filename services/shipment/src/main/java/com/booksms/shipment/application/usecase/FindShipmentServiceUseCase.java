...
public class FindShipmentServiceUseCase {
    ...
    public List<ShipmentServiceModel> execute(List<Criteria> criteriaList) {
        List<ShipmentService> shipmentServices;

        if(criteriaList == null || criteriaList.isEmpty()) {
            shipmentServices = shipmentServiceRepository.findAll();
        }
        ...
        return shipmentServices.stream().map(mapperUseCase::toModel).toList();
    }
